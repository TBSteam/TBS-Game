...
func load(path: String, original_path: String):
	...
	if err != OK:
		push_error("For some reason, loading LDTK resource failed with error code: %s"%err)
		return err
	
	var json_data := JSON.parse(file.get_as_text())
	
	if json_data.error:
		push_error("Failed parsing LDTK file: [%s at line %s]" % [json_data.error_string, json_data.error_line])
